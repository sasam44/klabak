...
- **`quoteRiskParams`**: Supplies the portfolio-risk inputs for the vault reserve model. `probabilityWad` is the top-tier win probability in WAD (`1e18 = 100%`). `maxPayout` is the worst-case total player payout, `expectedPayout` is the RTP-based mean payout, and `bodyVarianceScaled` is the **body variance**: the variance of the round's payout with the top tier removed, per bet, in the reserve's scaled units (wei² × 1e18). It is counted for every bet; only a game whose sole winning outcome is the top tier returns `0`. See `SLOTS_RISK_AND_RESERVES.md`.
...
2. Pin the session's per-bet variance — quoted body variance plus the top-tier binary term from `(maxPayout, probabilityWad)`, raised to any council-registered per-game σ floor — and add it into the vault aggregate.
...
- **Slots and any multi-tier paytable** must return the body variance in `bodyVarianceScaled`; a slot whose top multiplier is above the heavy-tail threshold cannot be whitelisted while it quotes `0` unless the security council registers a σ floor for it.
...
| Entry                                                                        | Role                                                                                                                                                                                                                                                                                                                                 |
| ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `openSession(game, vault, wager, gameData)`                                  | Validates whitelist, vault, min bet, caps from `quoteCaps`, risk inputs from `quoteRiskParams`, updates the casino portfolio reserve, transfers wager in, creates session, runs `onSessionStart`, then applies `StepResult` (may request RNG, wait for player, or finalize). An overload takes a trailing `operator` address (§7.1). |
...
### 7.1 Operators (not open yet)

An **operator** is a frontend that serves players on the shared vaults. It earns a share of the
house revenue from the sessions it opens. The operator code is simply the operator's address.

**Status: operator registration is closed.** Only the protocol's own operator is registered today,
so every session is attributed to it and nothing changes for your game or your host. Registration
...
- **Naming the operator:** the facet has a second entry point,
  `openSession(game, vault, wager, gameData, operator)`. The four-argument call keeps working and
  means "no operator code".
- **No code, or an unregistered code, never reverts.** The session is attributed to the protocol's
  default operator instead, so a stale host config cannot block a bet.
- **The code cannot be swapped in transit.** It is part of the `openSession` calldata, which the
  player's Smart Vault signs as one batch. A relayer that changes it invalidates the signature.
- **Gas sponsorship:** the chain.wtf relayer pays gas only for sessions that name no operator or
  chain.wtf's own. An operator sponsors its own players by running its own relayer.
- **Games see nothing.** The operator is not part of `SessionContext` and does not reach the
  iframe. It is committed inside the encoded session (20 bytes after the risk flags), so a host that
  decodes session bytes itself must use the current `CasinoSessionCodec` layout.
