...
From the SDK package root, a workspace covering the simulator, the VRF node, the slot engine and
the example game (bun shown; npm, pnpm and yarn work the same):

```sh
bun install   # installs every workspace package in one go
bun start     # local chain + VRF node + deployment + harness (:3300) + coinflip example (:3100)
```
...
`simulator/`: `bun run local-node` (chain + VRF + deployment) and `bun run dev` (harness only).
